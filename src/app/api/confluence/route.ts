...
// Fetch candles from Binance PAXGUSDT
...
export async function GET(request: Request) {
  try {
    ...
    const levels = gannLevels(price).map(level => {
      ...
      if (level > price && ema9 < ema21) score += 1;
      return { level, score: Math.min(score, 10), type: level < price ? 'BUY' as const : 'SELL' as const, signals: ['Gann'] };
    }).sort((a, b) => b.score - a.score).slice(0, 5);

    // Save to DB (fire and forget)
    supabase.from('scans').insert({ timeframe: tf, price, trend, levels: levels.length, top_score: levels[0]?.score ?? 0 }).then(() => {});

    return NextResponse.json({
      symbol: 'XAUUSD (PAXG)', timeframe: tf, price, trend, candles: candles.length,
      rsi: Math.round(rsiVal * 100) / 100,
      macd: { line: Math.round(macdVal.line * 10000) / 10000, signal: Math.round(macdVal.signal * 10000) / 10000, histogram: Math.round(macdVal.histogram * 10000) / 10000 },
